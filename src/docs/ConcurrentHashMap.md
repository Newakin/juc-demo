...
```
sizeCtl = [initialCapacity + 0.5initialCapacity + 1] 然后向上取2的n次方。
如果initialCapacity = 10, sizeCtl = 16;  
如果initialCapacity = 11, sizeCtl = (1.5*11+1=17.5) -> 32;  
...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K,V>[] tab = table;;) {
        ...
        // 扩容
            ...
            if (binCount != 0) {
                // 判断是否要将链表转换为红黑树，临界值和 HashMap 一样，链表长度是 8
...
2.如果Node没有初始化，则调用initTable进行初始化  
3.如果Node位置没有数据则直接插入  
4.判断是否需要扩容  
5.以上都不满足，对应的位置已经有数据了，则进行synchronized操作  
6.如果是链表，则插入链表；如果是红黑树则插入红黑树  
7.如果链表长度大于8，则转换为红黑树  
8.调用addCount，检查是否需要扩容  
